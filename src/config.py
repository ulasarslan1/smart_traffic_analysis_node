...
class RadarConfig:
    ...
    def load_config(self) -> None:
        """Load radar parameters from the JSON configuration file."""
        ...
        try:
            ...
            self.chirp_duration_s = float(
                radar["chirp_duration_s"]
            )
            self.sampling_frequency_hz = float(
                radar["sampling_frequency_hz"]
            )
            self.target_range_m = float(
                radar["target_range_m"]
            )
    ...
    def validate(self) -> None:
        ...
        if self.chirp_duration_s <= 0:
            raise ValueError(
                "chirp_duration_s must be greater than zero."
            )
        ...
        if self.target_range_m < 0:
            raise ValueError(
                "target_range_m cannot be negative."
            )
